package Facebook.Task_Scheduler_621;


//solution1: 有序的，最正常的task schedule：输出的是最后的时间 (不能sort)

//solution2: 有序的，最正常的task schedule：输出的是字符串 '_'

//solution3: 无序的，频率统计的做法，算最后时间

//solution4: 无序的，统计频率的做法，输出最后字符串

//soltion5: 有序的，if cooldown is very small, but there are lots of tasks, how to reduce space? O(cooldown) space

import java.util.*;
//n = 2
//the interval betwwen two A will be 3,
//A**A**A

//get each word's exist frequency from the string first,
//because each words need to be separate n cycles, therefore the
//minimum length between most frequency element is n, but we can add other less frequent element between two most frequency element,
//therefore, the finial result will be task.length, or c[25] - 1 * (n+1) +25 - i

//c[25]-1: the most frequent letter happen time minus 1
//*(n+1): count for the loops for above
//+25-i: count for the most frequence letters.
...
//1，最正常的task schedule：输出的是最后的时间
    ...
    //in hashmap, the key is task, the value is next available cycle for this task (map.put(task, slots + 1 + cooldown));
    //if the current slot is bigger than next available time fot this task, then we don't need to add cooldown time
    //otherwise, we need to add cooldown time for this task
...
//5，//if cooldown is very small, but there are lots of tasks, how to reduce space? O(cooldown) space
    ...
    // if we meet a new task, we only care if previous of this task still in the cooldown time or not, the cooldown time is constant,
    // then we have a queue, to keep it size of the cooldown time, if the task is in the queue, then we know this task is still in the cool down time
    //if the queue reach the cooltime number, if we need to remove one task from the queue, and also remove this task from the hashmap, which means the cooldown
    //of this task is end
    private int taskSchedule5(int[] tasks, int cooldown) {
        if (tasks == null || tasks.length == 0) {
        return 0;
        }
